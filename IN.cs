    ...
    internal static class IN
    {
        internal static int Main_Page(ref All_Info all_info)
        {
            ...
            while (true)
            {
                ...
                Drawer.Print_Inscription(all_info.window.cords.x + 3, all_info.window.cords.y + 1, $"Добро пожаловать, {all_info.all_doctors[all_info.window.index_user].Full_name}!");
        ...
        internal static int Weekly_Schedule(ref All_Info all_info)
        {
            ...
            while (true)
            {
                ...
                foreach (Patient pat in all_info.all_doctors[all_info.window.index_user].patients)
        ...
        internal static void Day_Schedule(ref All_Info all_info, int day)
        {
            ...
            while (true)
            {
                ...
                max_counter_tab = all_info.all_doctors[all_info.window.index_user].patients.Count * 2 + 2;
                
                Drawer.Print_Inscription(all_info.window.cords.x + 15, all_info.window.cords.y + 2, $"Расписание на {weekday[day]}");

                int i = 0;
                foreach (var pat in all_info.all_doctors[all_info.window.index_user].patients)
                {
                    if (pat.day_of_the_week == day)
                    {
                ...
                if (consoleKeyInfo.Key == ConsoleKey.Enter)
                {
                    ...
                    {
                        all_info.all_doctors[all_info.window.index_user].patients.RemoveAt(counter_tab / 2 - 1);
                        is_change_pic = true;
                    }

                    else 
                    {
                        var pat = all_info.all_doctors[all_info.window.index_user].patients[counter_tab / 2 - 1];
                        Change_Patient(ref all_info.window, ref pat);
                        all_info.all_doctors[all_info.window.index_user].patients[counter_tab / 2 - 1] = pat;
                        is_change_pic = true;
                    }
                    counter_tab = 0;
                }
        ...
        internal static void Add_Patient( ref All_Info all_info, int day)
        {
            ...
            if (Windows_Action.Yes_No_Buttons(new Point(all_info.window.cords.x + 5, all_info.window.cords.y + 23), new Point(all_info.window.cords.x + 25, all_info.window.cords.y + 23),
               "Назад", "Ок"))
                all_info.all_doctors[all_info.window.index_user].patients.Add(new Patient(time, time_2, name, day));


        }


        internal static void Change_Patient(ref Window window, ref Patient patient)
        {
